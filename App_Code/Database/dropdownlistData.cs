...
/// <summary>
/// Summary description for dropdownlistData
/// </summary>
public class dropdownlistData
{
    ...
    public dropdownlistData(int Id)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@int_Id", Id));
        using (DataSet ds = connect.GetDataset("SELECT * FROM dropdownlist WHERE id=@int_Id", param))
        {
    ...
    public dropdownlistData(string name)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@name", name));
        using (DataSet ds = connect.GetDataset("SELECT * FROM dropdownlist WHERE name=@name", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                HasValue = true;
    ...
    public void Save()
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO dropdownlist(name,address,user_id,status,image) VALUES(@name,@address,@user_id,@status,@image)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        param.Add(new MySqlParameter("@image", _Image));
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE dropdownlist SET name=@name,address=@address,user_id=@user_id,status=@status,image=@image WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
