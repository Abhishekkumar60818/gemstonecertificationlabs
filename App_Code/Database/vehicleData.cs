...
public class vehicleData
{
    ...
    public vehicleData(int Id)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@int_Id", Id));
        using (DataSet ds = connect.GetDataset("SELECT * FROM vehicleddb WHERE id=@int_Id", param))
        {
    ...
    public vehicleData(string name)
    {
        ...
        using (DataSet ds = connect.GetDataset("SELECT * FROM vehicleddb WHERE name=@name", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                HasValue = true;
    ...
    public void Save()
    {
        ...
        param.Add(new MySqlParameter("@contact", Rent));

        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO vehicleddb(name,address,user_id,status,image,contact) VALUES(@name,@address,@user_id,@status,@image,@contact)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        param.Add(new MySqlParameter("@contact", Rent));
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE vehicleddb SET name=@name,address=@address,user_id=@user_id,image=@image,contact=@contact WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
