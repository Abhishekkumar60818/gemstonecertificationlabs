...
/// <summary>
/// Summary description for MedicineTypeData
/// </summary>
public class MedicineTypeData
{
    private int _Id;
	...
	public MedicineTypeData()
	{
		//
		// TODO: Add constructor logic here
		//
	}
      public MedicineTypeData(int Id)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@int_Id", Id));
        using (DataSet ds = connect.GetDataset("SELECT * FROM medicine_type WHERE id=@int_Id", param))
        {
      ...
      public MedicineTypeData(string name)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@name", name));
        using (DataSet ds = connect.GetDataset("SELECT * FROM medicine_type WHERE name=@name", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                HasValue = true;
    ...
    public void Save()
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO medicine_type(name,address,user_id,status,image) VALUES(@name,@address,@user_id,@status,@image)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        param.Add(new MySqlParameter("@image", _Image));
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_type SET name=@name,address=@address,user_id=@user_id,status=@status,image=@image WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
