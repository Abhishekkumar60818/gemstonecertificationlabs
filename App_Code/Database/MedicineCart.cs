...
/// <summary>
/// Summary description for MedicineCart
/// </summary>
public class MedicineCart
{
	...
	public MedicineCart()
	{
      ...
      public MedicineCart(int id, string sessionid,string dose,string email)
    {
        string query = "";
          if(email!="")
          {
              UserData udata=new UserData(email);
              query = "SELECT * FROM medicine_cart WHERE product_id=@int_Id and sessionid=@sessionid and dose=@dose and userid="+udata.Id;
          }
          else{
              query="SELECT * FROM medicine_cart WHERE product_id=@int_Id and sessionid=@sessionid and dose=@dose and userid=0";
          }
      ...
      public MedicineCart(int id, string sessionid, string dose, string email, string noUse)
      {
          string query = "";
          if (email != "")
          {
    ...
    public MedicineCart(int Id)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@int_Id", Id));
        using (DataSet ds = connect.GetDataset("SELECT * FROM medicine_cart WHERE id=@int_Id", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                HasValue = true;
    ...
    public MedicineCart(string sessionid, int userid)
    {
        Connection connect = new Connection();
        List<MySqlParameter> param = new List<MySqlParameter>();
        param.Add(new MySqlParameter("@sessionid", sessionid));
        param.Add(new MySqlParameter("@userid", userid));
        using (DataSet ds = connect.GetDataset("SELECT * FROM medicine_cart WHERE sessionid=@sessionid or userid=@userid ", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                HasValue = true;
    ...
    public void Save()
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO medicine_cart(product_id,dose,price,new_price,quantity,sessionid,userid,user_type) VALUES(@productid,@dose,@price,@newprice,@quantity,@sessionid,@userid,@usertype)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_cart SET quantity=@quantity WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int pid,string sessionid,int uid)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_cart SET quantity=@quantity WHERE product_id=@pid and sessionid=@sessionid and userid=@uid", param);
        connect.Dispose();
        connect = null;
    }
    public void Update(string sessionid)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_cart SET userid=@userid WHERE sessionid=@sessionid and userid=0", param);
        connect.Dispose();
        connect = null;
    }
    public void UpdateOldSession(string oldSessionid)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_cart SET sessionid=@newSessionId WHERE sessionid=@oldsessionid and userid=0", param);
        connect.Dispose();
        connect = null;
    }
    public void UpdateByUser(int userid)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_cart SET sessionid=@sessionid,user_type=@user_type WHERE userid=@userid", param);
        connect.Dispose();
        connect = null;
    }
    public void UpdateUserPrice(int id)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE medicine_cart SET new_price=@new_price WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
