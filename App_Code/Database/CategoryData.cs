...
public class CategoryData
{
    ...
    public CategoryData(int Id)
    {
        ...
        using (DataSet ds = connect.GetDataset("SELECT * FROM category WHERE id=@int_Id", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                _Category = ds.Tables[0].Rows[0]["category"].ToString();
    ...
    public void Save()
    {
       ...
       // param.Add(new MySqlParameter("@date", _Date));
        ...
        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO category(category,description,image,pagetitle,metakey,metadescription) VALUES(@category,@description,@image,@pagetitle,@metakey,@metadescription)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE category SET category=@category,description=@description,image=@image,pagetitle=@pagetitle,metakey=@metakey,metadescription=@metadescription WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
