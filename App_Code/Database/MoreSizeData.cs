...
public class MoreSizeData
{
    ...
    public MoreSizeData(int Id)
    {
        ...
        using (DataSet ds = connect.GetDataset("SELECT * FROM productdetail WHERE id=@int_Id", param))
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                _Image = ds.Tables[0].Rows[0]["vimage2"].ToString();
                _Image = ds.Tables[0].Rows[0]["vimage3"].ToString();
    ...
    public void Save()
    {
        ...
        param.Add(new MySqlParameter("@vimage2", _Image2));
        param.Add(new MySqlParameter("@vimage3", _Image3));
        ...
        Connection connect = new Connection();
        connect.ExecStatement("INSERT INTO productdetail(subcategoryid,productid,vsize,vprice,vdiscount,vnewprice,vquantity,vimage,vimage2,vimage3) VALUES(@subcategoryid,@productid,@vsize,@vprice,@vdiscount,@vnewprice,@vquantity,@vimage,@vimage2,@vimage3)", param);
        connect.Dispose();
        connect = null;
    }
    ...
    public void Update(int id)
    {
        ...
        param.Add(new MySqlParameter("@vquantity", _Quantity));
        param.Add(new MySqlParameter("@vimage", _Image));
        param.Add(new MySqlParameter("@vimage2", _Image));
        param.Add(new MySqlParameter("@vimage3", _Image));
    
        Connection connect = new Connection();
        connect.ExecStatement("UPDATE productdetail SET subcategoryid=@subcategoryid,productid=@productid,vsize=@vsize,vprice=@vprice,vdiscount=@vdiscount,vnewprice=@vnewprice,vquantity=@vquantity,vimage=@vimage,vimage2=@vimage2,vimage3=@vimage3WHERE id=@id", param);
        connect.Dispose();
        connect = null;
    }
